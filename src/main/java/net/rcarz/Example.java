...
public class Example {
  ...
  public static void main(String[] args) throws JiraException {
    ...
    try {
       ...
       * fails.
       */
      final Issue issue = jira.getIssue("HH-16101");
      ...
      /* Assign the issue */
      issue.update().field(Field.ASSIGNEE, "nick").execute();
      ...
      /*
       * Change the summary and add two labels to the issue. The double-brace
       * initialiser
      ...
      List<CustomFieldOption> allowedValues =
          jira.getCustomFieldAllowedValues("customfield_5678", "TEST", "Task");
      ...
      /* Create a new issue. */
      Issue newIssue = jira.createIssue("HH", "Task").field(Field.SUMMARY, "Bat signal is broken")
          .field(Field.DESCRIPTION, "Commissioner Gordon reports the Bat signal is broken.")
          .field(Field.ASSIGNEE, "nick").field(Field.FIX_VERSIONS, new ArrayList() {
            {
              add("7.4");
            }
       ...
       */
      Issue.SearchResult src = jira.searchIssues("project IN (GOTHAM) ORDER BY id");
      while (src.iterator().hasNext())
        System.out.println("Result: " + src.iterator().next());
