...
public abstract class AgileResource {
   ...
   * @throws JiraException when the retrieval fails
   */
  protected static <T extends AgileResource> T getResource(Class<T> type, Object r,
      RestClient restclient) throws JiraException {
    ...
    if (!((JSONObject) r).isNullObject()) {
      try {
        Constructor<T> constructor =
            type.getDeclaredConstructor(RestClient.class, JSONObject.class);
   ...
   * @throws JiraException when the retrieval fails
   */
  protected static <T extends AgileResource> List<T> getResourceArray(Class<T> type, Object ra,
      RestClient restclient, String listName) throws JiraException {
   ...
   * @throws JiraException when the retrieval fails
   */
  static <T extends AgileResource> List<T> list(RestClient restclient, Class<T> type, String url)
      throws JiraException {
    return list(restclient, type, url, "values");
  }

  static <T extends AgileResource> List<T> list(RestClient restclient, Class<T> type, String url,
      Map<String, String> params) throws JiraException {
    JSON result;
    try {
      result = restclient.get(url, params);
    } catch (Exception ex) {
      throw new JiraException("Failed to retrieve a list of " + type.getSimpleName() + " : " + url,
          ex);
    }

    return getResourceArray(type, result, restclient, "values");
  }
   ...
   */
  static <T extends AgileResource> List<T> list(RestClient restclient, Class<T> type, String url,
      String listName) throws JiraException {
    ...
    return getResourceArray(type, result, restclient, listName);
  }
   ...
   * @throws JiraException when the retrieval fails
   */
  static <T extends AgileResource> T get(RestClient restclient, Class<T> type, String url)
      throws JiraException {
    ...
    return getResource(type, result, restclient);
  }
   ...
   * @throws JiraException when the retrieval fails
   */
  <T extends AgileResource> List<T> getSubResourceArray(Class<T> type, JSONObject subJson,
      String resourceName) throws JiraException {
    List<T> result = null;
    if (subJson.containsKey(resourceName)) {
      result =
          getResourceArray(type, subJson.get(resourceName), getRestclient(), resourceName + "s");
    }
    return result;
  }
   ...
   * @throws JiraException when the retrieval fails
   */
  <T extends AgileResource> T getSubResource(Class<T> type, JSONObject subJson, String resourceName)
      throws JiraException {
    T result = null;
