...
public class Issue extends Resource {
  ...
  public static final class FluentCreate {
     ...
     *                       <li>summary,comment - include just the summary and
     *                       comments</li>
  ...
  /**
   * Used to {@link #create() create} remote links. Provide at least the
   * {@link #url(String)} or
  ...
  public static final class FluentRemoteLink {
    ...
    /**
     * A globally unique identifier which uniquely identifies the remote
     * application and the remote object within
     * the remote system. The maximum length is 255 characters. This call sets
     * also the {@link #url(String) url}.
     *
     ...
     * 
     * @param url   A 16x16 icon representing the type of the object in the
     *              remote system.
     * @param title Text for the tooltip of the main icon describing the type of
     *              the object in the remote system.
     * @return this instance
     */
     ...
     * 
     * @param resolved  if {@code true} the link to the issue will be in a
     *                  strike through font.
     * @param title     Text for the tooltip of the main icon describing the
     *                  type of the object in the remote system.
     * @param iconUrl   Text for the tooltip of the main icon describing the
     *                  type of the object in the remote system.
     * @param statusUrl A hyperlink for the tooltip of the the status icon.
     * @return this instance
     */
    public FluentRemoteLink status(final boolean resolved, final String iconUrl, final String title,
    ...
    /**
     * Relationship between the remote object and the JIRA issue. This can be a
     * verb or a noun.
     * It is used to group together links in the UI.
     * 
     * @param relationship Relationship between the remote object and the JIRA
     *                     issue.
     * @return this instance
     */
    ...
    /**
     * The application for this remote link. Links are grouped on the type and
     * name in the UI. The name-spaced
     * type of the application. It is not displayed to the user. Renderering
     * plugins can register to render a
     * certain type of application.
     * 
     * @param type The name-spaced type of the application.
     * @param name The human-readable name of the remote application instance
     *             that stores the remote object.
     * @return this instance
     */
    ...
    /**
     * Creates or updates the remote link if a {@link #globalId(String) global
     * id} is given and there is already
    ...
    private Transition getTransition(String id, boolean isName) throws JiraException {
      ...
      if (result == null) {
        ...
        throw new JiraException(
            "Transition '" + id + "' was not found. Known transitions are:" + allTransitionNames);
      }
  ...
  private static class IssueIterator implements Iterator<Issue> {
    ...
    public IssueIterator(RestClient restclient, String jql, String includedFields,
        String expandFields, Integer maxResults, Integer startAt) throws JiraException {
      this.restclient = restclient;
      this.jql = jql;
    ...
    public void remove() {
      throw new UnsupportedOperationException(
          "Method remove() not support for class " + this.getClass().getName());
    }
    ...
    private List<Issue> getNextIssues() throws JiraException {
      ...
      try {
        URI searchUri =
            createSearchURI(restclient, jql, includedFields, expandFields, maxResults, startAt);
        result = restclient.get(searchUri);
      } catch (Exception ex) {
        throw new JiraException("Failed to search issues", ex);
      }
   ...
   *
   * The first is to access the issues field directly. This is a list of Issue
   * instances.
   * Note however that this will only contain the issues fetched in the initial
   * search,
   * so its size will be the same as the max result value or below.
   *
   * The second way is to use the iterator methods. This will return an Iterator
   * instance,
   * that will iterate over every result of the search, even if there is more
   * than the max
   * result value. The price for this, is that the call to next has none
   * determistic performence,
  ...
  public static class SearchResult {
    ...
    public SearchResult(RestClient restclient, String jql, String includedFields,
        String expandFields, Integer maxResults, Integer startAt) throws JiraException {
      this.issueIterator =
          new IssueIterator(restclient, jql, includedFields, expandFields, maxResults, startAt);
  ...
  private void deserialise(JSONObject json) {
    ...
    attachments =
        Field.getResourceArray(Attachment.class, fields.get(Field.ATTACHMENT), restclient);
  ...
  public static JSONObject getCreateMetadata(RestClient restclient, String project,
    ...
    List<Project> projects =
        Field.getResourceArray(Project.class, (JSONArray) jo.get("projects"), restclient);
   ...
   * @param timeSpentSeconds provided time spent. This cannot be lower than 1m
   *                         inute
   * @return
  ...
  public WorkLog addWorkLog(String comment, DateTime startDate, long timeSpentSeconds)
    ...
    try {
      ...
      if (timeSpentSeconds < 60) // We do not add a worklog that duration is
                                 // below a minute
      ...
      req.put("started",
          DateTimeFormat.forPattern(Field.DATETIME_FORMAT).print(startDate.getMillis()));
  ...
  /**
   * Links this issue with another issue and adds a comment with limited
   * visibility.
   *
   * @param issue   Other issue key
   * @param type    Link type name
  ...
  public static FluentCreate create(RestClient restclient, String project, String issueType)
    ...
    FluentCreate fc =
        new FluentCreate(restclient, getCreateMetadata(restclient, project, issueType));
  ...
  public FluentCreate createSubtask() throws JiraException {
    return Issue.create(restclient, getProject().getKey(), "Sub-task").field(Field.PARENT,
        getKey());
  }

  private static JSONObject realGet(RestClient restclient, String key,
      Map<String, String> queryParams) throws JiraException {
   ...
   *                       </ul>
   *
   * @param maxResults     if non-<code>null</code>, defines the maximum number
   *                       of
   ...
   */
  private static URI createSearchURI(RestClient restclient, String jql, String includedFields,
      String expandFields, Integer maxResults, Integer startAt) throws URISyntaxException {
    Map<String, String> queryParams = new HashMap<String, String>();
  ...
  public boolean delete(final boolean deleteSubtasks) throws JiraException {
    ...
    try {
      URI uri =
          restclient.buildURI(getBaseUri() + "issue/" + this.key, new HashMap<String, String>() {
            {
